...
bb_bigram_count <- bb_bigram_count %>%
    mutate( bigram =  do.call(paste, c(bb_bigram_count[c("chord", "next_chord")], sep = " ")) ) %>% 
...
bb_bigram_count %>%
  ...
  ggplot(aes(x = bigram, y = share, fill = chord)) +
  geom_bar(stat = 'identity') +
  coord_flip() +
  xlab('chord changes') +
  ylab('share of total chords') 
...
bb_tagg <- bb_tag %>%
    mutate( bigram =  do.call(paste, c(bb_tag[c("chord", "next_chord")], sep = " ")) ) %>% 
    filter(title == next_title) %>%
    count(bigram, sort = TRUE)
